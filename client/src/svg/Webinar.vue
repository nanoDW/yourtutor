<template>
  <svg
    class="webinar"
    :class="{ 'active-icon': currentStep === 4 }"
    height="480pt"
    viewBox="0 -16 480 480"
    width="480pt"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path d="m448 272h-416v-240h416zm-400-16h384v-208h-384zm0 0" />
    <path d="m120 296h312v16h-312zm0 0" />
    <path d="m96 288h16v32h-16zm0 0" />
    <path d="m48 296h40v16h-40zm0 0" />
    <path d="m184 228.945312v-153.890624l153.886719 76.945312zm16-128v102.109376l102.113281-51.054688zm0 0" />
    <path
      d="m456 0h-432c-13.253906 0-24 10.746094-24 24v304c0 13.253906 10.746094 24 24 24h144v48h-80v48h304v-48h-80v-48h144c13.253906 0 24-10.746094 24-24v-304c0-13.253906-10.746094-24-24-24zm-80 416v16h-272v-16zm-80-16h-112v-48h112zm168-72c0 4.417969-3.582031 8-8 8h-432c-4.417969 0-8-3.582031-8-8v-304c0-4.417969 3.582031-8 8-8h432c4.417969 0 8 3.582031 8 8zm0 0"
    />
  </svg>
</template>

<script>
export default {
  props: ["currentStep"],
};
</script>

<style lang="scss" scoped>
@import "../styles/variables";
@import "../styles/transitions";

.webinar {
  position: absolute;
  bottom: 20%;
  left: 20%;
  width: 100px;
  height: 100px;
  fill: $secondary-dark;
  opacity: 0.1;
}

.active-icon {
  transform: translateX(-100px) translateY(10px);
  animation: animate-icon 3s linear infinite;
  @extend .reset-absolute;
}

@keyframes animate-icon {
  from {
    transform: rotate(90deg) translateX(250px) rotate(270deg);
  }
  to {
    transform: rotate(450deg) translateX(250px) rotate(-90deg);
  }
}
</style>
